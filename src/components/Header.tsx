...
export default function Header() {
  ...
  const virtualAssistantSubLinks = [
    { href: "/services/virtual-assistant/real-estate", label: "Real Estate" },
    { href: "/services/virtual-assistant/coaches-trainers", label: "Coaches & Trainers" },
    { href: "/services/virtual-assistant/social-media-video", label: "Social Media & Video" },
    { href: "/services/virtual-assistant/construction-&-contractors", label: "Construction & Contractors" },
  ...
  const services: NavItem[] = [
    { href: "/services/virtual-assistant", label: "Virtual Assistant", children: virtualAssistantSubLinks },
  ...
  // ✅ considers /services/virtual-assistant/* as active too
  const isParentActive = (href: string, children?: NavItem["children"]) => {
                    ...
                    <div key={s.href} className="relative group/va">
                      ...
                      <Link
                        ...
                        className={`flex items-center justify-between gap-2 px-4 py-2.5 text-sm transition-colors ${
                          isParentActive(s.href, s.children)
                    ...
                    s.children?.length ? (
                      <div key={s.href}>
                        ...
                        <Link
                          href={s.href}
                          onClick={() => setOpen(false)}
                          className={`block pl-8 pr-4 py-2.5 text-sm rounded-lg transition-all ${
                            isParentActive(s.href, s.children)
                        ...
                        {/* Children */}
                        <div className="mt-1 ml-8 pl-3 border-l border-[#E2DAD0] space-y-1">
                          {s.children.map((child) => (
                            <Link
                              key={child.href}
                              href={child.href}
                              onClick={() => setOpen(false)}
                              className={`block pr-4 py-2 text-sm rounded-lg transition-all ${
                                pathname === child.href
                                  ? "text-[#161412] font-medium bg-[#F0EBE1]"
                                  : "text-[#7A7168] hover:text-[#161412] hover:bg-[#F9F6F0]"
                              }`}
                            >
                              {child.label}
